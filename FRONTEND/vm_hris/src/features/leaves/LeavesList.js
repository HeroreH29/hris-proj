...
import { useGetEmployeeRecordsQuery } from "../employeerecords/recordsApiSlice";
...
const LeavesList = () => {
  ...
  const handleHover = ({ leaveCredit = "", name = "" }) => {
    try {
      if (!leaveCredit && !name) {
        return setLeaveCredit("");
      }
      setLeaveCredit({ ...leaveCredit, name });
    } catch (error) {
      console.log(error);
    }
  };
  ...
  if (isSuccess) {
    ...
    // Filter records with non-empty leave lists
    const recordsWithLeaves = ids
      .filter((id) => entities[id].Leaves.length)
      .map((id) => entities[id]);

    const leaveList = [];
    recordsWithLeaves.forEach((record) => {
      record.Leaves.forEach((leave) => {
        // Clone the leave object and add FullName
        const leaveWithFullName = {
          ...leave,
          FullName: record.GenInfo.FullName,
        };
        leaveList.push({
          ...leaveWithFullName,
          LeaveCredits: record.LeaveCredits,
        });
      });
    });

    const filteredLeaves = leaveList.filter((leave) => {
      const employeeName = leave.FullName.toLowerCase();
      const filedYear = new Date(leave.DateFiled).getFullYear();
      const filedMonth = new Date(leave.DateFiled).toLocaleDateString(
        "default",
        { month: "short" }
      );
    ...
    overallLeavesContent = filteredLeaves?.length
      ? filteredLeaves
          .reduce((acc, leave) => {
            if (leave.Approve === 0) {
              // Insert leaves with Approve === 0 to the beginning of the array
              acc.unshift(leave);
            } else {
              // Append other leaves to the end of the array
              acc.push(leave);
            }
            return acc;
          }, [])
          .sort((a, b) => {
            // Sort by date
            const dateA = new Date(a.DateFiled).valueOf();
            const dateB = new Date(b.DateFiled).valueOf();
            if (tableState.dateSort) {
              return dateA - dateB;
            } else {
              return dateB - dateA;
            }
          ...
          .map((leave) => (
            <Leave
              ...
              leaveCredit={leave.LeaveCredits}
            />
          ))
      : null;
            ...
            >
              <caption>
                {isHR || isAdmin || isOutletProcessor
                  ? `Leave Credit Info of ${leaveCredit.name}`
                  : "Your Credit Info"}
              </caption>
              <thead>
                <tr>
